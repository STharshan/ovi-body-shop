const ServiceHero = {
    DetailHero: {
        badgeText: "Professional Detailing",
        title: "Detailing –",
        highlight: "Polish & Finishing",
        description: "Professional vehicle detailing and finishing services to restore and protect your car’s appearance. Our expert technicians provide meticulous care for a showroom-quality shine inside and out.",
        phone: "+441623623759"
    },
    InsuranceHero: {
        badgeText: "Insurance Approved Repairer",
        title: "Insurance Jobs –",
        highlight: "Claim Support",
        description: "Hassle-free insurance repair services with expert claim support. We handle your vehicle repairs from start to finish, working directly with insurers to ensure smooth, stress-free processing.",
        phone: "+441623623759"
    }, AccidentHero: {
        badgeText: "Certified Body Shop",
        title: "Accident Repairs –",
        highlight: "Minor to Major Damage",
        description: "Comprehensive accident repair services for vehicles of all sizes. From small fender-benders to major collision damage, our expert technicians restore your car to full safety and aesthetic standards.",
        phone: "+441623623759"
    },
    DentRepairHero: {
        badgeText: "24-Hour Turnaround",
        title: "Dent &",
        highlight: "Scratch Repair",
        description: "Fast and professional dent and scratch repairs to restore your vehicle’s appearance. Our expert technicians provide high-quality repairs with minimal downtime, so your car looks perfect in just one day.",
        phone: "+441623623759"
    },
    PaintResprayHero: {
        badgeText: "Advanced Blowtherm Tech",
        title: "Professional",
        highlight: "Paint Resprays",
        description: "High-quality paint resprays using advanced Blowtherm technology for a flawless, durable finish. Restore your vehicle’s original shine and colour with precision painting and expert surface preparation.",
        phone: "+441623623759"
    }, BodyworkHero: {
        badgeText: "Expert Craftsmanship",
        title: "Panel Beating &",
        highlight: "Bodywork Restoration",
        description: "Professional panel beating and bodywork restoration using precision repair techniques and modern equipment. From minor dents to full body restoration, we bring your vehicle back to factory-standard finish with expert craftsmanship.",
        phone: "+441623623759"
    }
}

export default ServiceHero;